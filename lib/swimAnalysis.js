...
const buildCoachingTip = (ctx, tr) => {
  ...
  return tr('feedback.tipDefault');
};

const buildCoachNarrative = (ctx, tr) => {
  const {
    isFirst,
    m,
    daysSinceLast,
  ...
  if (hasPb) {
    ...
    parts.push(tr('feedback.coachRecovery'));
  } else {
    parts.push(tr('feedback.welcomeBack', {
...
export const buildPersonalFeedback = (newSession, allSessions, t, profile = {}) => {
  ...
  const ctx = gatherFeedbackContext(newSession, allSessions, profile, base, combined);
  ...
  let coachMessage = buildCoachNarrative(ctx, tr);
