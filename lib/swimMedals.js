...
/** First session date that unlocks each medal (chronological walk). */
export const computeEarnedAtMap = (sessions) => {
  ...
  for (let i = 0; i < sorted.length; i += 1) {
    ...
    const ctx = buildMedalContext(slice);
    MEDALS.forEach((medal) => {
      if (found.has(medal.id)) return;
      const { earned } = evaluateMedal(medal, ctx);
      if (earned) {
        earnedAt[medal.id] = sorted[i].date;
        found.add(medal.id);
      }
    });
    if (found.size === MEDALS.length) break;
  }

  return earnedAt;
};

export const evaluateAllMedals = (sessions, options = {}) => {
  const { allMedalsUnlocked = false } = options;
  const ctx = buildMedalContext(sessions);
  const earnedAtMap = computeEarnedAtMap(sessions);
  const today = new Date().toISOString().slice(0, 10);

  return MEDALS.map((medal) => {
    const result = evaluateMedal(medal, ctx);
    const earned = allMedalsUnlocked || result.earned;
    return {
      ...medal,
      ...result,
      earned,
      earnedAt: earned ? (earnedAtMap[medal.id] ?? today) : null,
      progress: earned ? null : getMedalProgress(medal, ctx),
    };
  });
};
...
export const getNewlyEarnedMedals = (sessionsBefore, sessionsAfter, options = {}) => {
  const beforeEarned = new Set(
    evaluateAllMedals(sessionsBefore, options).filter((m) => m.earned).map((m) => m.id)
  );
  return evaluateAllMedals(sessionsAfter, options).filter((m) => m.earned && !beforeEarned.has(m.id));
};

/** @internal */
export const __testing = { buildMedalContext, getWeekKey, getSeason, getNewlyEarnedMedals, computeEarnedAtMap };
