...
/**
 * Resolve the active mascot id from the profile.
 * Falls back to the legacy `mascotSex` field ('male' → flip, 'female' → flo)
 * and finally the swimmer's own sex.
 */
export function resolveMascotId(profile) {
  if (profile?.mascotId && MASCOTS[profile.mascotId]) return profile.mascotId;
  const legacy = profile?.mascotSex || profile?.sex;
  if (legacy === 'female') return 'flo';
  return 'flip';
}
