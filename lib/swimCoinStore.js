...
export const STORE_CATEGORIES = ['themes', 'vibes', 'flair', 'boosts', 'titles'];
...
export function purchaseStoreItemUpdate(id, storeUnlocks, totalCoins) {
...
export function getBonusPaidSpins(storeUnlocks = []) {
  return isStoreItemOwned('wheel:bonus-spin', storeUnlocks) ? 1 : 0;
}

export function getDailyPaidSpinLimit(storeUnlocks = []) {
  return 3 + getBonusPaidSpins(storeUnlocks);
}
...
export function getOwnedCosmeticUnlocks(storeUnlocks = []) {
  return normalizeStoreUnlocks(storeUnlocks).filter(
    (id) => id.startsWith('ambient:') || id.startsWith('title:')
  );
}
