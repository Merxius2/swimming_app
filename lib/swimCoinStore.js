...
export const BONUS_SPIN_BASE_PRICE = 350;
export const BONUS_SPIN_PRICE_INCREMENT = 150;
...
export function getBonusSpinPrice(bonusWheelSpinCredits = 0) {
  const owned = Math.max(0, Number(bonusWheelSpinCredits) || 0);
  return BONUS_SPIN_BASE_PRICE + owned * BONUS_SPIN_PRICE_INCREMENT;
}

export function getConsumableItemPrice(id, context = {}) {
  const item = getStoreItem(id);
  if (!item) return 0;
  if (id === BONUS_WHEEL_SPIN_STORE_ITEM_ID) {
    return getBonusSpinPrice(context.bonusWheelSpinCredits);
  }
  return item.price;
}
...
export function canPurchaseStoreItem(id, storeUnlocks, totalCoins, context = {}) {
  if (!CATALOG_IDS.has(id)) return false;
  const item = getStoreItem(id);
  if (item.consumable) return (totalCoins ?? 0) >= getConsumableItemPrice(id, context);
...
export function purchaseConsumableStoreItemUpdate(id, totalCoins, coinsSpent = 0, context = {}) {
  const item = getStoreItem(id);
  const price = getConsumableItemPrice(id, context);
  if (!item?.consumable || (totalCoins ?? 0) < price) return null;
  return {
    totalCoins: Math.max(0, (totalCoins ?? 0) - price),
    coinsSpent: Math.max(0, (coinsSpent ?? 0) + price),
  };
}
