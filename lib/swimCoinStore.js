import { normalizeMascotEquipped, resolveMascotSex } from './mascotConstants.js';
...
export const STORE_CATEGORIES = ['themes', 'icons', 'vibes', 'flair', 'boosts', 'titles', 'mascot'];
...
export const STORE_CATALOG = [
  ...
  },
  {
    ...
    category: 'titles',
    price: 100,
    preview: 'title-lane-seven',
    nameKey: 'coins.store.items.laneSeven.name',
    descKey: 'coins.store.items.laneSeven.desc',
  },
  {
    id: 'title:pool-shark',
    category: 'titles',
    price: 150,
    preview: 'title-pool-shark',
    nameKey: 'coins.store.items.poolShark.name',
    descKey: 'coins.store.items.poolShark.desc',
  },
  {
    id: 'title:splash-zone',
    category: 'titles',
    price: 125,
    preview: 'title-splash-zone',
    nameKey: 'coins.store.items.splashZone.name',
    descKey: 'coins.store.items.splashZone.desc',
  },
  {
    id: 'mascot:neon-cap',
    category: 'mascot',
    price: 150,
    preview: 'mascot-neon-cap',
    nameKey: 'coins.store.items.mascotNeonCap.name',
    descKey: 'coins.store.items.mascotNeonCap.desc',
  },
  {
    id: 'mascot:gold-goggles',
    category: 'mascot',
    price: 200,
    preview: 'mascot-gold-goggles',
    nameKey: 'coins.store.items.mascotGoldGoggles.name',
    descKey: 'coins.store.items.mascotGoldGoggles.desc',
  },
  {
    id: 'mascot:medal-chain',
    category: 'mascot',
    price: 300,
    preview: 'mascot-medal-chain',
    nameKey: 'coins.store.items.mascotMedalChain.name',
    descKey: 'coins.store.items.mascotMedalChain.desc',
  },
  {
    id: 'mascot:party-hat',
    category: 'mascot',
    price: 175,
    preview: 'mascot-party-hat',
    nameKey: 'coins.store.items.mascotPartyHat.name',
    descKey: 'coins.store.items.mascotPartyHat.desc',
  },
  {
    id: 'mascot:snorkel',
    category: 'mascot',
    price: 225,
    preview: 'mascot-snorkel',
    nameKey: 'coins.store.items.mascotSnorkel.name',
    descKey: 'coins.store.items.mascotSnorkel.desc',
  },
  {
    id: 'mascot:champion-cape',
    category: 'mascot',
    price: 400,
    preview: 'mascot-champion-cape',
    nameKey: 'coins.store.items.mascotChampionCape.name',
    descKey: 'coins.store.items.mascotChampionCape.desc',
  },
  {
    id: 'mascot:suit-classic',
    category: 'mascot',
    price: 180,
    preview: 'mascot-suit-classic',
    mascotSex: 'female',
    nameKey: 'coins.store.items.mascotSuitClassic.name',
    descKey: 'coins.store.items.mascotSuitClassic.desc',
  },
  {
    id: 'mascot:suit-racing',
    category: 'mascot',
    price: 250,
    preview: 'mascot-suit-racing',
    mascotSex: 'female',
    nameKey: 'coins.store.items.mascotSuitRacing.name',
    descKey: 'coins.store.items.mascotSuitRacing.desc',
  },
  {
    id: 'mascot:suit-tropical',
    category: 'mascot',
    price: 200,
    preview: 'mascot-suit-tropical',
    mascotSex: 'female',
    nameKey: 'coins.store.items.mascotSuitTropical.name',
    descKey: 'coins.store.items.mascotSuitTropical.desc',
  },
  {
    id: 'mascot:shorts-classic',
    category: 'mascot',
    price: 180,
    preview: 'mascot-shorts-classic',
    mascotSex: 'male',
    nameKey: 'coins.store.items.mascotShortsClassic.name',
    descKey: 'coins.store.items.mascotShortsClassic.desc',
  },
  {
    id: 'mascot:shorts-jammer',
    category: 'mascot',
    price: 250,
    preview: 'mascot-shorts-jammer',
    mascotSex: 'male',
    nameKey: 'coins.store.items.mascotShortsJammer.name',
    descKey: 'coins.store.items.mascotShortsJammer.desc',
  },
  {
    id: 'mascot:shorts-sunset',
    category: 'mascot',
    price: 200,
    preview: 'mascot-shorts-sunset',
    mascotSex: 'male',
    nameKey: 'coins.store.items.mascotShortsSunset.name',
    descKey: 'coins.store.items.mascotShortsSunset.desc',
  },
];
...
/** @deprecated use normalizeStoreUnlocks */
export function normalizePurchasedThemes(raw) {
  return Object.keys(LEGACY_THEME_IDS).filter((code) => Array.isArray(raw) && raw.includes(code));
}
...
/** @deprecated */
export function purchaseThemeUpdate(themeCode, storeUnlocks, totalCoins) {
  return purchaseStoreItemUpdate(`theme:${themeCode}`, storeUnlocks, totalCoins);
}
...
export function sanitizeProfileCosmetics(profile, storeUnlocks = []) {
  ...
  if (next.swimmerTitle && !isStoreItemOwned(next.swimmerTitle, storeUnlocks)) {
    next.swimmerTitle = null;
  }
  if (next.activeAppIcon && !isStoreItemOwned(next.activeAppIcon, storeUnlocks)) {
    next.activeAppIcon = null;
  }
  if (Array.isArray(next.mascotEquipped)) {
    next.mascotEquipped = normalizeMascotEquipped(
      next.mascotEquipped,
      storeUnlocks,
      resolveMascotSex(next)
    );
  } else {
    next.mascotEquipped = [];
  }
...
export function getOwnedCosmeticUnlocks(storeUnlocks = []) {
  return normalizeStoreUnlocks(storeUnlocks).filter(
    (id) => id.startsWith('ambient:') || id.startsWith('title:') || id.startsWith('icon:') || id.startsWith('mascot:')
  );
}

export function getOwnedMascotItems(storeUnlocks = []) {
  return normalizeStoreUnlocks(storeUnlocks).filter((id) => id.startsWith('mascot:'));
}

export function getSwimmerTitleLabel(titleId, t) {
