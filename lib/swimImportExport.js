...
export const generateSwimExportString = async (swimData) => {
  const payload = {
    ...
    pt: normalizePurchasedThemes(swimData.purchasedThemes),
  };
...
export const parseSwimImportString = async (str) => {
  ...
  if (compressed.v !== 3 && compressed.v !== 4 && compressed.v !== 5) {
  ...
  return {
    ...
    spentCoinClaims: compressed.v >= 4
      ...
      : [],
    purchasedThemes: compressed.v >= 5
      ? normalizePurchasedThemes(compressed.pt)
      : [],
  };
};
