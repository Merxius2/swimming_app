import { normalizeStoreUnlocks } from './swimCoinStore.js';
import { STROKE_KEYS } from './swimConstants.js';

const normalizeStrokes = (raw = {}) => {
  const strokes = {};
  STROKE_KEYS.forEach((key) => {
    const value = raw[key];
    strokes[key] = value != null && Number.isFinite(Number(value)) ? Number(value) : null;
  });
  return strokes;
};
...
const compressSession = (session) => ({
  ...
  ce: session.coinsEarned,
  cb: session.coinBonus,
  ...(session.excludeFromStats ? { ex: true } : {}),
  m: {
...
const decompressSession = (raw) => ({
  ...
  date: raw.dt,
  createdAt: raw.ca,
  coinsEarned: raw.ce ?? null,
  coinBonus: raw.cb ?? null,
  excludeFromStats: Boolean(raw.ex),
  metrics: {
    ...
    location: raw.m?.loc ?? '',
    timeRange: raw.m?.tr ?? '',
    strokes: normalizeStrokes(raw.m?.st),
  },
});

const compressCoinClaim = (claim) => ({
  dt: claim.date,
  m: {
    dm: claim.metrics?.distanceM,
    ds: claim.metrics?.durationSec,
    ps: claim.metrics?.paceSecPer100m,
    tr: claim.metrics?.timeRange || '',
  },
});

const decompressCoinClaim = (raw) => ({
  date: raw.dt,
  metrics: {
    distanceM: raw.m?.dm ?? null,
    durationSec: raw.m?.ds ?? null,
...
export const generateSwimExportString = async (swimData) => {
  const payload = {
    v: EXPORT_VERSION,
    tc: swimData.totalCoins ?? 0,
    cs: swimData.coinsSpent ?? 0,
    p: {
      sex: swimData.profile?.sex || 'male',
      age: swimData.profile?.age ?? 30,
    },
    s: (swimData.sessions || []).map(compressSession),
    cc: (swimData.spentCoinClaims || []).map(compressCoinClaim),
    su: normalizeStoreUnlocks(swimData.storeUnlocks, swimData.purchasedThemes),
    aa: swimData.profile?.activeAmbient || null,
    aic: swimData.profile?.activeAppIcon || null,
  };
...
export const parseSwimImportString = async (str) => {
  ...
  if (compressed.v !== 3 && compressed.v !== 4 && compressed.v !== 5 && compressed.v !== 6 && compressed.v !== 7 && compressed.v !== 8 && compressed.v !== 9) {
    throw new Error('Unsupported export version');
  }

  const legacyThemes = compressed.v >= 5 ? (compressed.pt || []) : [];
  const storeUnlocks = compressed.v >= 6
    ? normalizeStoreUnlocks(compressed.su, legacyThemes)
    : normalizeStoreUnlocks([], legacyThemes);
  const coinsSpent = compressed.v >= 8
    ? (compressed.cs ?? 0)
    : undefined;

  return {
    profile: {
      sex: compressed.p?.sex || 'male',
      age: compressed.p?.age ?? 30,
      aiApiKey: '',
      activeAmbient: compressed.v >= 6 ? (compressed.aa || null) : null,
      activeAppIcon: compressed.v >= 7 ? (compressed.aic || null) : null,
    },
    totalCoins: compressed.tc ?? 0,
    coinsSpent,
    sessions: (compressed.s || []).map(decompressSession),
    spentCoinClaims: compressed.v >= 4
      ? (compressed.cc || []).map(decompressCoinClaim)
      : [],
    storeUnlocks,
  };
};
