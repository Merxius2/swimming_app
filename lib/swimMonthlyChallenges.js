...
export const normalizeMonthRerollEntry = (entry) => {
  if (!entry || typeof entry !== 'object') {
    return { overrides: {}, freeUsed: false };
  }
  if (typeof entry.tierIndex === 'number' && entry.type) {
    return { overrides: { [entry.tierIndex]: entry.type }, freeUsed: true };
  }
  return {
    overrides: entry.overrides && typeof entry.overrides === 'object' ? { ...entry.overrides } : {},
    freeUsed: Boolean(entry.freeUsed),
  };
};
...
export const isFreeMonthlyRerollUsed = (monthKey, rerolls = {}) => (
  normalizeMonthRerollEntry(rerolls[monthKey]).freeUsed
);

export const hasRerollAvailability = (monthKey, rerolls = {}, credits = 0) => (
  !isFreeMonthlyRerollUsed(monthKey, rerolls) || credits > 0
);

const applyRerollOverrides = (challenges, monthEntry, stats) => {
...
export const generateMonthlyChallenges = (sessions, monthKey, rerolls = {}) => {
  ...
  return applyRerollOverrides(challenges, rerolls[monthKey], stats);
};
...
export const canRerollMonthlyChallenge = (
  ...
  if (!hasRerollAvailability(monthKey, rerolls, credits)) return false;
