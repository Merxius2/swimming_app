...
const pickRerollType = (monthKey, tierIndex, candidates) => {
  if (!candidates.length) return null;
  let s = hashMonth(`${monthKey}:reroll:${tierIndex}`) || 1;
  s = (s * 1103515245 + 12345) & 0x7fffffff;
  return candidates[s % candidates.length];
};

const applyRerollOverride = (challenges, override, stats) => {
  if (!override || typeof override.tierIndex !== 'number' || !override.type) return challenges;
  const { tierIndex, type } = override;
  if (tierIndex < 0 || tierIndex >= challenges.length) return challenges;
...
export const generateMonthlyChallenges = (sessions, monthKey, rerolls = {}) => {
  ...
  return applyRerollOverride(challenges, rerolls[monthKey], stats);
};

/** @returns {{ tierIndex: number, type: string } | null} */
export const createMonthlyChallengeReroll = (sessions, monthKey, tierIndex) => {
  if (tierIndex < 0 || tierIndex > 2) return null;
  const base = generateMonthlyChallenges(sessions, monthKey);
  ...
  const type = pickRerollType(monthKey, tierIndex, candidates);
  if (!type) return null;
  return { tierIndex, type };
};

export const hasMonthlyChallengeReroll = (monthKey, rerolls = {}) => Boolean(rerolls[monthKey]);

export const canRerollMonthlyChallenge = (sessions, monthKey, tierIndex, rerolls = {}) => {
  if (hasMonthlyChallengeReroll(monthKey, rerolls)) return false;
  const state = evaluateMonthlyChallenges(sessions, monthKey, rerolls);
  const challenge = state.challenges[tierIndex];
  if (!challenge || challenge.completed) return false;
  return Boolean(createMonthlyChallengeReroll(sessions, monthKey, tierIndex));
};
...
export const evaluateMonthlyChallenges = (sessions, monthKey = getMonthKey(), rerolls = {}) => {
  const definitions = generateMonthlyChallenges(sessions, monthKey, rerolls);
...
export const getMonthlyTierUpgrade = (
  ...
  const before = evaluateMonthlyChallenges(sessionsBefore, monthKey, rerolls);
  const after = evaluateMonthlyChallenges(sessionsAfter, monthKey, rerolls);
...
export const getMonthlyChallengeHistory = (
  sessions,
  { previewMonthlyMedals = false, monthlyChallengeRerolls = {} } = {}
) => {
  const months = [...new Set(sessions.map((s) => monthKeyFromDate(s.date)))].sort().reverse();
  const real = months
    .map((monthKey) => evaluateMonthlyChallenges(sessions, monthKey, monthlyChallengeRerolls))
    .filter((m) => m.tier);
