...
export function getPaidSpinsRemaining(wheelSpins, today = getWheelSpinDayKey()) {
  const used = getPaidSpinsUsedToday(wheelSpins, today);
  return Math.max(0, DAILY_PAID_SPIN_LIMIT - used);
}

export function canUsePaidSpin(wheelSpins, today = getWheelSpinDayKey()) {
  return getPaidSpinsRemaining(wheelSpins, today) > 0;
}
...
export function canStartWheelSpin(totalCoins, bet, freeSpins, wheelSpins, today = getWheelSpinDayKey()) {
  if (freeSpins > 0) return true;
  if (!canUsePaidSpin(wheelSpins, today)) return false;
