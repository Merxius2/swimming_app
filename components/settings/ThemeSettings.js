...
import { getUnlockedThemes, isThemeUnlocked } from '../../lib/swimCoinStore';
import ThemedIcon from '../ThemedIcon';

export default function ThemeSettings() {
  const { t } = useLanguage();
  const { theme, changeTheme, THEMES, isLoading: prefsLoading } = useTheme();
  const { storeUnlocks, isLoading: swimLoading, cheats } = useSwim();
  const isLoading = prefsLoading || swimLoading;
  const allThemesUnlocked = Boolean(cheats?.allThemesUnlocked);

  const visibleThemes = useMemo(
    () => getUnlockedThemes(THEMES, storeUnlocks, allThemesUnlocked),
    [THEMES, storeUnlocks, allThemesUnlocked]
  );

  const handleSelectTheme = (code) => {
    if (!isThemeUnlocked(code, storeUnlocks, allThemesUnlocked)) return;
    changeTheme(code);
  };
          ...
          {visibleThemes.map((item) => {
            ...
            return (
              <button
                key={item.code}
                type="button"
                onClick={() => handleSelectTheme(item.code)}
