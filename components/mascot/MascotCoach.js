...
/**
 * Flip/Flo/Fins swim coach: character with speech bubble, name, and level badge.
...
export default function MascotCoach({
  ...
  const content = (
    <div className={`flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-6 ${className}`}>
      <div className="order-2 sm:order-1 shrink-0 flex flex-col items-center mascot-enter">
        <MascotCharacter mascotId={mascotId} mood={mood} size={size} animated={animated} />
      ...
      <div className="order-1 sm:order-2 flex-1 min-w-0 w-full flex flex-col items-center sm:items-start gap-2 sm:pb-8">
        {(coachName || showLevelBadge) && (
          <div className="mascot-coach-header flex items-center gap-2.5 flex-wrap justify-center sm:justify-start">
            {coachName && (
              <p className="mascot-coach-name text-xs font-bold uppercase tracking-wider text-ink-soft">
                {coachName}
              </p>
            )}
  ...
  return (
    <MascotStage mascotId={mascotId} className="px-4 py-5 sm:px-6 sm:py-6">
      {content}
    </MascotStage>
  );
}
